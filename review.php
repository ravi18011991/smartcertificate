...
/**
 * This page reviews a smartcertificate
 *
...
$PAGE->set_url('/mod/smartcertificate/review.php', array('id' => $cm->id));
...
if (!$certrecord = $DB->get_record('smartcertificate_issues', array('userid' => $USER->id, 'smartcertificateid' => $smartcertificate->id))) {
    notice(get_string('nosmartcertificatesissued', 'smartcertificate'), "$CFG->wwwroot/course/view.php?id=$course->id");
    die;
}

// Load the specific smartcertificatetype
...
$reviewurl = new moodle_url('/mod/smartcertificate/review.php', array('id' => $cm->id));
...
echo html_writer::tag('p', get_string('viewed', 'smartcertificate'). '<br />' . userdate($certrecord->timecreated), array('style' => 'text-align:center'));

$link = new moodle_url('/mod/smartcertificate/review.php?id='.$cm->id.'&action=get');
$linkname = get_string('reviewsmartcertificate', 'smartcertificate');
