...
if (!$users = smartcertificate_get_issues($smartcertificate->id, $DB->sql_fullname(), $groupmode, $cm, $page, $perpage)) {
    ...
    echo $OUTPUT->notification(get_string('nosmartcertificatesissued', 'smartcertificate'));
...
if ($download == "ods") {
    ...
    if ($users) {
        foreach ($users as $user) {
            ...
            $myxls->write_string($row, $nextposition + 2, smartcertificate_get_grade($smartcertificate, $course, $user->id));
...
if ($download == "txt") {
    $filename = smartcertificate_get_smartcertificate_filename($smartcertificate, $cm, $course) . '.txt';
    ...
    if ($users) foreach ($users as $user) {
        ...
        echo smartcertificate_get_grade($smartcertificate, $course, $user->id) . "\t";
        echo $user->code . "\n";
        $row++;
    }
    exit;
...
foreach ($users as $user) {
    ...
    $date = userdate($user->timecreated) . smartcertificate_print_user_files($smartcertificate, $user->id, $context->id);
    $code = $user->code;
    $data = array();
    $data[] = $name;
    foreach ($extrafields as $field) {
    ...
    $data = array_merge($data, array($date, smartcertificate_get_grade($smartcertificate, $course, $user->id), $code));
    $table->data[] = $data;
}
...
echo $OUTPUT->header();
groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/smartcertificate/report.php?id='.$id);
echo $OUTPUT->heading(get_string('modulenameplural', 'smartcertificate'));
