...
/**
 * A4_embedded smartcertificate type
 *
...
smartcertificate_print_text($pdf, $x, $y + 20, 'C', $fontserif, '', 20, get_string('certify', 'smartcertificate'));
smartcertificate_print_text($pdf, $x, $y + 36, 'C', $fontsans, '', 30, fullname($USER));
smartcertificate_print_text($pdf, $x, $y + 55, 'C', $fontsans, '', 20, get_string('statement', 'smartcertificate'));
smartcertificate_print_text($pdf, $x, $y + 72, 'C', $fontsans, '', 20, format_string($course->fullname));
smartcertificate_print_text($pdf, $x, $y + 92, 'C', $fontsans, '', 14,  smartcertificate_get_date($smartcertificate, $certrecord, $course));
smartcertificate_print_text($pdf, $x, $y + 102, 'C', $fontserif, '', 10, smartcertificate_get_grade($smartcertificate, $course));
smartcertificate_print_text($pdf, $x, $y + 112, 'C', $fontserif, '', 10, smartcertificate_get_outcome($smartcertificate, $course));
if ($smartcertificate->printhours) {
    smartcertificate_print_text($pdf, $x, $y + 122, 'C', $fontserif, '', 10, get_string('credithours', 'smartcertificate') . ': ' . $smartcertificate->printhours);
}
...
if ($smartcertificate->printteacher) {
    ...
    if ($teachers = get_users_by_capability($context, 'mod/smartcertificate:printteacher', '', $sort = 'u.lastname ASC', '', '', '', '', false)) {
        foreach ($teachers as $teacher) {
            $i++;
            smartcertificate_print_text($pdf, $sigx, $sigy + ($i * 4), 'L', $fontserif, '', 12, fullname($teacher));
        }
    }
}
